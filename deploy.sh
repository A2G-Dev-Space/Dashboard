...
wait_healthy() {
  ...
  while [ $elapsed -lt $max_wait ]; do
    ...
    if [ "$health" = "no-healthcheck" ]; then
      # 헬스체크 없는 서비스 (dashboard-frontend) — 컨테이너 running이면 OK
      local state
...
switch_upstream() {
  ...
  # 백업 (롤백용)
  BACKUP_UPSTREAM=$(cat nginx/upstream.conf)
...
rollback_upstream() {
  if [ -n "$BACKUP_UPSTREAM" ]; then
    warn "upstream 롤백 중..."
    echo "$BACKUP_UPSTREAM" > nginx/upstream.conf
...
cmd_init() {
  ...
  # Nginx — upstream.conf를 blue로 설정 후 시작
  log "4/4 Nginx 시작"
  cat > nginx/upstream.conf << 'CONF'
# Auto-generated by deploy.sh (active: blue)
