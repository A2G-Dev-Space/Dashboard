 ...
 * 피드백 상세 조회
 */
feedbackRoutes.get('/:id', authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Authentication required' });
            return;
 ...
 * 피드백 수정 (본인만)
 */
feedbackRoutes.put('/:id', authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Authentication required' });
            return;
 ...
 * 피드백 삭제 (본인 또는 Admin)
 */
feedbackRoutes.delete('/:id', authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Authentication required' });
            return;
 ...
 * 피드백에 답변 달기 (Admin만)
 */
feedbackRoutes.post('/:id/respond', authenticateToken, requireAdmin, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Authentication required' });
            return;
 ...
 * 피드백 상태 변경 (Admin만)
 */
feedbackRoutes.patch('/:id/status', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
 ...
 * 피드백에 댓글 추가 (Admin만)
 */
feedbackRoutes.post('/:id/comments', authenticateToken, requireAdmin, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Authentication required' });
            return;
 ...
 * 댓글 수정 (본인 댓글만)
 */
feedbackRoutes.put('/:id/comments/:commentId', authenticateToken, requireAdmin, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Authentication required' });
            return;
 ...
 * 댓글 삭제 (본인 댓글만)
 */
feedbackRoutes.delete('/:id/comments/:commentId', authenticateToken, requireAdmin, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({ error: 'Authentication required' });
            return;
 ...
 */
async function checkIsAdmin(loginid) {
