...
| 서버 IP | 포트 |
|---------|------|
| `12.81.222.45` | 13000, 4090, 443 |
| `12.81.222.41` | 36810, 443 |

**자동 업데이트 제한사항:**
- 위 방화벽 해제만으로는 **자동 업데이트가 지원되지 않습니다.**
- 자동 업데이트는 `13000` 포트로 **100MB 이상 통신이 가능한 환경**에서만 동작합니다.
- 자동 업데이트가 불가한 경우 [다운로드 페이지](/nexus-coder-windows)에서 최신 설치 파일을 직접 다운로드하여 설치하세요.
:::
...
1. 다운로드한 `Nexus Coder (For Windows)-Setup-5.0.2.exe` 실행
2. 설치 경로 선택 (기본값 권장)
3. **Install** 클릭
